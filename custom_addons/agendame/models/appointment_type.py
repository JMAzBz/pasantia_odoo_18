...
class AppointmentType(models.Model):
    ...
    def _get_appointment_slots(self, reference_date=None):
        ...
        is_today = reference_date is None or reference_date == fields.Date.today()
            ...
            busy_events = (
                ...
                .search_read(
                    [
                        ...
                        "|",
                        ("agendame_status", "!=", "cancelled"),
                        ("agendame_status", "=", False),
                    ],
        ...
        for i in range(max_days):
            ...
            for start_hour, end_hour in day_slots:
                ...
                while current_hour + duration <= end_hour:
                    ...
                    if is_today and check_date == fields.Date.today() and utc_dt <= now_utc:
    ...
    # ------------------------------------------------------------------
    # Per-staff availability (individual professional booking)
    # ------------------------------------------------------------------

    def _get_appointment_slots_for_staff(self, staff_user, reference_date=None):
        """
        Generate available slots for a SINGLE staff user.
        Returns a list of UTC datetimes where only that user is free.
        """
        self.ensure_one()
        now_utc = fields.Datetime.now()
        is_today = reference_date is None or reference_date == fields.Date.today()
        ...
        for event in busy_events:
            ...
            busy_periods.append((event_start, event_stop))
        ...
        for i in range(max_days):
            ...
            for start_hour, end_hour in day_slots:
                ...
                while current_hour + duration <= end_hour:
                    ...
                    # Check if the staff user is free
                    is_busy = False
                    for event_start, event_stop in busy_periods:
                        if event_start < slot_end_utc and event_stop > utc_dt:
                            is_busy = True
                            break
    ...
    def _is_slot_available_for_staff(self, staff_user, start_dt, end_dt):
        """
        Check if a specific staff user is free in this time range.
        """
        self.ensure_one()
        partner_id = staff_user.partner_id.id
        if not partner_id:
